// Water shader: same lighting/fog as chunks, but translucent with a subtle sun sheen.
...
fn fs_main(in: VsOut) -> @location(0) vec4<f32> {
    ...
    // Specular highlight for a watery sheen.
    let view_dir = normalize(camera.cam_pos.xyz - in.world_pos);
    let halfway = normalize(sun + view_dir);
    let spec = pow(max(dot(n, halfway), 0.0), 64.0) * sun_intensity * 0.6;

    var rgb = in.color * light + vec3<f32>(spec);
